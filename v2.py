...
DOCKERFILE_PATH = 'Dockerfile (2).txt'  # Define explicitly
...
def load_database():
    ...
    if os.path.exists(DATABASE_FILE):
        with open(DATABASE_FILE, 'r') as f:
            for line in f:
                if line.strip():
                    vps_id, owner_id, memory, cpu, username, ssh, status, created_at, expiry, token = line.strip().split('\t')
                    vps_data[vps_id] = {
...
def save_database():
    with open(DATABASE_FILE, 'w') as f:
        for vps_id, data in vps_data.items():
            line = f"{vps_id}\t{data['owner_id']}\t{data['memory']}\t{data['cpu']}\t{data['username']}\t{data['full_ssh']}\t{data['status']}\t{data['created_at']}\t{data['expiry'] or 'None'}\t{data['token']}\n"
            f.write(line)
...
async def create_vps_with_os(ctx, os_type, memory, cpu, username, expiry=None):
    ...
    try:
        # Run Docker commands in background
        ...
        ssh_command = await capture_ssh_session_line(exec_proc)
        if not ssh_command:
    ...
    vps_data[vps_id] = {
        ...
        'cpu': cpu,
        'username': username,
        'full_ssh': ssh_command or f"ssh {username} {ssh_command}",
        'status': 'Running',
...
async def create_vps(ctx, memory: str, cpu: str, username: str, expiry: str = None):
    ...
    view = OSSelectView(ctx, memory, cpu, username, expiry_date)
    await ctx.send("Select an OS for your VPS:", view=view)
...
async def manage_vps(ctx, vps_id: str, action: str = None, os_type: str = None):
    ...
    if action.lower() == 'start':
        try:
            subprocess.Popen(["docker", "start", container_name]).wait()
        ...
        try:
            subprocess.Popen(["docker", "stop", container_name]).wait()
        ...
        try:
            subprocess.Popen(["docker", "restart", container_name]).wait()
            vps_data[vps_id]['status'] = 'Running'
            proc = await asyncio.create_subprocess_exec(
                "docker", "exec", container_name, "tmate", "-F",
                stdout=asyncio.subprocess.PIPE
        ...
        if os.path.exists(DOCKERFILE_PATH):
            try:
                subprocess.Popen(["docker", "stop", container_name]).wait()
                subprocess.Popen(["docker", "rm", container_name]).wait()
                build_proc = subprocess.Popen([
                    "docker", "build", "-t", container_name,
                    "-f", DOCKERFILE_PATH, "."
                ])
...
async def recreate_vps(ctx, old_vps_id: str):
    ...
    try:
        subprocess.Popen(["docker", "stop", old_container]).wait()
        subprocess.Popen(["docker", "rm", old_container]).wait()
        build_proc = subprocess.Popen([
...
async def help(ctx):
    help_message = """
Available commands:
- !create_vps <ram> <cpu> <username> <timeexiper> - Create a new VPS (e.g., !create_vps 1 1 testuser 1h)
- !recreate_vps <vps_id> - Recreate an existing VPS
- !manage_vps <vps_id> [action] - Manage VPS (actions: Start, Stop, Restart, Status, Reinstall)
- !nodes - Show node status
- !send_vps <user_id> <username> <ssh_pass> - Send VPS details to a user
- !addadmin_bot <user_id> - Add a new admin
- !delete_vps <vps_id> <username> - Delete a VPS
- !vpslist - List your VPS instances
